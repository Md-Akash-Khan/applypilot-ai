...
async function main() {
  ...
  console.log(`ApplyPilot scheduled scan started at ${startedAt.toISOString()}`);
  ...
  for (const source of sources) {
    ...
    try {
      ...
      console.log(`[SUCCESS] ${source.name}: ${result.discovered} discovered, ${result.created} imported, ${result.skipped} skipped.`);
    } catch (error) {
      failed += 1;
      console.error(`[FAILED] ${source.name}:`, error);
    }
>>>>>>> 1b00594 (Upgrade ApplyPilot AI to version 2)
  }

  console.log(`Completed: ${scanned} scanned, ${skipped} not due, ${imported} jobs imported, ${failed} failed.`);
